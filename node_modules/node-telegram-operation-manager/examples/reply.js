...
const reply_manager_1 = require("../src/reply_manager");
const node_telegram_keyboard_wrapper_1 = require("node-telegram-keyboard-wrapper");
//*************************//
//* EXECUTE THIS, TRUST ME*//
//*************************//
const reply = new reply_manager_1.default();
const keyboards = [
    new node_telegram_keyboard_wrapper_1.ReplyKeyboard("I hate replies"),
    (new node_telegram_keyboard_wrapper_1.ReplyKeyboard()).addRow("I hate replies", "I love replies"),
    new node_telegram_keyboard_wrapper_1.ReplyKeyboard("Show me potatoes!")
];
bot_1.default.onText(/\/multireply/, (message) => {
    bot_1.default.sendMessage(message.from.id, `Hey there! 😎 Some replies got registered! Follow my instructions to discover how this works. Send me a message now!`);
    ...
    reply
        ...
        bot_1.default.sendMessage(message.from.id, "Great! Now send me another message.");
    })
        .register(message.from.id, (someData) => {
        ...
        bot_1.default.sendMessage(message.from.id, nextText, messageOpts);
    })
        .register(message.from.id, (someData) => {
        let nextText;
        ...
        if (someData.text === "I love replies") {
            ...
            bot_1.default.sendMessage(message.from.id, nextText, messageOpts2);
            return {
            ...
            bot_1.default.sendMessage(message.from.id, nextText, messageOpts1);
            return { repeat: true };
        }
    })
        .register(message.from.id, (someData) => {
        if (someData.text !== "Show me potatoes!") {
            bot_1.default.sendMessage(message.from.id, "Nope! The next is not correct! Try again.");
            return { repeat: true };
        }
        ...
        bot_1.default.sendMessage(message.from.id, nextText, Object.assign({}, keyboards[2].close(), { parse_mode: "Markdown" }));
    })
        .register(message.from.id, (someData) => {
        bot_1.default.sendMessage(message.from.id, "You are the best! Start now by looking at the [documentation](https://github.com/alexandercerutti/node-telegram-operation-manager#class_reply). 😉 Hope you have enjoyed the tutorial!", { parse_mode: "Markdown" });
    });
});
